      ...
      <div className={cn('flex items-start gap-4 p-6')}>
        <div className="relative h-16 w-16 shrink-0">
          <Image
            ...
            width={64}
            height={64}
            className={cn('rounded-lg object-cover', 'border border-border bg-background')}
          />
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                {isVerified ? (
                  <Badge variant="success" className={cn('flex items-center gap-1 px-2 py-0')}>
                    <CheckCircle className="h-3 w-3" />
                    <span className="text-xs">Verified</span>
                  </Badge>
                ) : (
                  <Badge variant="secondary" className={cn('flex items-center gap-1 px-2 py-0')}>
                    <span className="text-xs">Not Verified</span>
                  </Badge>
                )}
              </div>
              <p className={cn('text-sm text-muted-foreground')}>{companyType}</p>
            </div>

            <Button variant="secondary" size="sm" className={cn('flex items-center gap-2')}>
              <CheckCircle className="h-4 w-4" />
              Following
            </Button>
          </div>

          <p className={cn('line-clamp-2 text-sm text-foreground')}>{description}</p>
